...
export function PostComments({ postId, isSignedIn }: PostCommentsProps) {
  ...
  const { mutate: createComment, loading: isSubmitting } = useMutation({
    collection: 'comment',
    action: 'create'
  })

  return (
    <QuerySwitcher collection="comment">
      {(query) => {
        const { data: allComments, isLoading: loading, error, refetch } = query
